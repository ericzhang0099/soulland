...
class BlockchainService {
    ...
    initialize() {
        try {
            ...
            if (this.contractAddress) {
                this.alipayContract = new ethers_1.ethers.Contract(this.contractAddress, ALIPAY_PAYMENT_HANDLER_ABI, this.signer);
            }
    ...
    updateContractAddress(address) {
        ...
        if (this.signer) {
            this.alipayContract = new ethers_1.ethers.Contract(address, ALIPAY_PAYMENT_HANDLER_ABI, this.signer);
        }
     ...
     * 在链上创建订单
     */
    async createOrderOnChain(userAddress, amountCny, metadata = '') {
        if (!this.isAvailable()) {
     ...
     */
    async confirmPayment(orderId, alipayOrderNo) {
        if (!this.isAvailable()) {
            throw new Error('区块链服务不可用');
        }
        try {
            // 将订单ID转换为 bytes32
            const orderIdBytes32 = orderId.startsWith('0x') ? orderId : ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes(orderId));
     ...
     * 批量确认支付
     */
    async batchConfirmPayments(orderIds, alipayOrderNos) {
        if (!this.isAvailable()) {
            throw new Error('区块链服务不可用');
        }
        ...
        try {
            const orderIdBytes32Array = orderIds.map(id => id.startsWith('0x') ? id : ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes(id)));
     ...
     * 计算可获得的积分数量
     */
    async calculatePoints(amountCny) {
        if (!this.isAvailable()) {
            throw new Error('区块链服务不可用');
        }
        try {
     ...
     * 检查支付宝订单号是否已使用
     */
    async isAlipayOrderUsed(alipayOrderNo) {
        if (!this.isAvailable()) {
            throw new Error('区块链服务不可用');
        }
        try {
     ...
     * 获取积分余额
     */
    async getPointsBalance(userAddress) {
     ...
     * 初始化积分合约（获取积分合约地址后调用）
     */
    async initializePointsContract() {
        if (!this.alipayContract || !this.signer) {
            throw new Error('支付宝合约未初始化');
        }
        try {
            this.pointsTokenAddress = await this.alipayContract.pointsToken();
            this.pointsContract = new ethers_1.ethers.Contract(this.pointsTokenAddress, GENLOOP_POINTS_ABI, this.signer);
