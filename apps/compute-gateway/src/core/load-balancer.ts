...
export class LoadBalancer {
  private latencyWeight = 0.3;
  private costWeight = 0.2;
  private availabilityWeight = 0.3;
  private loadWeight = 0.2;
  
  private maxLatencyMs = 5000;
  private maxErrorRate = 0.1;
  private maxLoad = 0.9;
  ...
  selectProvider(
    ...
    availableProviders: Map<string, ProviderStatus>,
  ...
  private filterCandidates(
    ...
    const candidates = new Map<string, ProviderStatus>();
    ...
    providers.forEach((status, name) => {
      ...
      if (status.errorRate > this.maxErrorRate) return;
      if (status.currentLoad > this.maxLoad) return;
      if (status.circuitState === 'open') return;
      
      // 模型支持检查
      if (!config.models.includes(request.model)) return;
  ...
  private calculateScores(candidates: Map<string, ProviderStatus>): Map<string, number> {
  ...
  private estimateCost(request: ChatCompletionRequest, config: ProviderConfig): number {
    // 估算token数
